<template>
  <div class="repairs">
    ...
    <el-steps :active="3" align-center style="margin:1em 0">
      <el-step title="接单" description="系统派单后需要24小时内完成接单" />
      <el-step title="反馈" description="工人需要到报修地进行初步检查，确认受损设备，然后在系统中进行反馈，上报故障设备，申请备件，及发送增派请求" />
      <el-step title="维修" description="工人进行维修，完成后上传完工图片备案" />
    </el-steps>
    <!-- /示意图 -->
    <!-- 表格 -->
    <el-table v-loading="loading" size="small" :data="workerData.list">
      ...
      <el-table-column label="工单类型" prop="type" width="80" align="center" />
      <el-table-column label="工单状态" width="120" align="center">
        <template slot-scope="scope">
          <el-tag type="success">{{ scope.row.status }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="工单负责人" prop="user.realname" width="100" />
      ...
      <el-table-column label="工单处理描述" prop="handlerContent" min-width="100" />
        ...
        <el-form ref="workerForm" :model="form" :rules="rules" label-width="80px">
          ...
          <div v-if="title === '反馈报修问题'">
            ...
            <el-form-item label="受损设备" prop="deviceId">
              <el-select v-model="form.deviceId">
                <el-option v-for="device in deviceData.list" :key="device.id" :label="device.name" :value="device.id" />
              </el-select>
            </el-form-item>
          ...
          <div v-if="title === '提交维修结果'">
            <el-form-item label="维修日志" prop="handlerContent">
              <el-input v-model="form.handlerContent" type="textarea" />
            </el-form-item>
    ...
    <el-dialog title="增派工单" :visible.sync="visible_increase" width="50%" class="customer_modal">
      <div class="content">
        ...
        <el-form label-width="100px">
          ...
          <el-form-item label="反馈描述" prop="checkContent">
            <el-input v-model="form_increase.checkContent" type="textarea" />
          </el-form-item>
        </el-form>
      </div>
      <span slot="footer" class="dialog-footer">
        ...
        <el-form label-width="100px">
          <el-form-item label="申请备件" prop="spareId">
            <el-select v-model="form_spare.spareId">
              <el-option v-for="spare in spareData.list" :key="spare.id" :label="spare.name" :value="spare.id" />
            </el-select>
          </el-form-item>
          <el-form-item label="申请数量" prop="spareNum">
            <el-input v-model="form_spare.spareNum" />
          </el-form-item>
        </el-form>
      </div>
      <span slot="footer" class="dialog-footer">
